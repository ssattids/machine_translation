...
class Add_Norm(nn.Module):
    def __init__(self, features=256, eps=1e-6):
...
#               print(preds_index[0,j])
        ...
        with open("loss3.txt", "a+") as l:
            l.write("Epoch: " + str(epoch) + " Loss: " + str(ep_loss) + " time: " + str(time.time()-start_time) + " Accuracy: " + str(accuracy / len(dataloader)) + "\n") 
        if epoch % 2 == 0:
            torch.save(encoder.state_dict(), './models3/t_encoder_' + str(params['hidden_size']) + '_' + str(epoch) + '.pt')
            torch.save(decoder.state_dict(), './models3/t_decoder_' + str(params['hidden_size']) + '_' + str(epoch) + '.pt')
